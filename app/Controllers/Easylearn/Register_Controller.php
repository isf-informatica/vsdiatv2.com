...
class Register_Controller extends ResourceController{
    ...
    public function forget_email_send_otp()
    {
        ...
        if($data)
        {
            ...
            $this->email->setSubject('OTP verification from VSD-IAT');
    ...
    public function add_mentor()
    {
        ...
        if(($data['m_token'] == $this->session->get('mentor_token')) && ($this->session->get('mentor_token') != NULL))
        {

            if(!$this->Register_Model->check_email($data['m_emailid']))
            {
                ...
                $result = $this->Register_Model->add_mentor($mentor,$accounts,$account_details);
                if($result){
                    $message = '
                                                                ...
                                                                <table width="100%" cellpadding="0" cellspacing="0">
                                                                    ...
                                                                    <tr>
                                                                        <td data-color="text" data-size="size text" data-min="10" data-max="26" data-link-color="link text color" data-link-style="font-weight:bold; text-decoration:underline; color:#40aceb;" align="center" style="font:bold 16px/25px Arial, Helvetica, sans-serif; color:#173965; padding:0 0 23px;">
                                                                            Your account verification is pending this may take up to 3 international banking days.
                                                                        </td>
                                                                    </tr>
                    ...
                    $this->email->setTo($data['m_emailid']);
                    $this->email->setFrom('info@isfedutech.com' , 'Vsdiat');
                    
                    $this->email->setSubject('WELCOME TO VSD-IAT');
                    $this->email->setMessage($message);
                    $this->email->send();
                    $array = array(
                        'Response' => 'OK',
